...
- Backend production packaging and test compilation pass locally. The local Docker engine is unavailable, so the authoritative PostgreSQL/Flyway/Testcontainers result is pushed Backend CI run `35527095739`: 260/260 tests pass and the production image builds successfully at backend closure commit `d5fcb71b48ef052616c8bd406c9cbe4eccaf6ff3`. The local full Maven attempt recorded 104 passing tests, zero assertion failures, and 29 Docker-unavailable errors.
- Android records 241/241 unit tests with zero failures, errors, or skips; lint has zero errors and 118 warnings; debug assembly, release Kotlin compilation, and the release bundle pass. Android CI run `35526366932` is green. The final bounded Pixel Tablet Android 15 run records 22/22 connected tests across conversation behavior, Room 9-to-10 migration, account isolation, offline queueing, retry, and idempotency. Six additional screenshot-capture scenarios pass.
- Xcode Cloud Build 109 for source `8105668a18a84fe5fa08b7933e6189cd6966473f` completes both `Build - iOS` and `Test - iOS` successfully. The test target contains 262 XCTest methods and no explicit skip calls; Xcode Cloud reports zero errors, test failures, analysis issues, or warnings. The iOS String Catalog contains 487 valid JSON keys. No TestFlight action was run.
- Beta release `8105668a18a84fe5fa08b7933e6189cd6966473f` runs as immutable image `phokarta-backend:8105668a18a84fe5fa08b7933e6189cd6966473f` (image digest `sha256:768700e264922b34bc5cee943f8e215b4aed0580d1291c70fb573da284b7c013`). Flyway applies V16 successfully; the backend is healthy with zero restarts; public and direct-origin liveness/readiness return HTTP 200; and the conversation capability is enabled. The pre-deployment PostgreSQL backup is `/opt/phokarta/backups/phokarta-beta-pre-m5-20260921T071846Z.dump` (95,621 bytes, SHA-256 `760d956cd993269dec5b76da95a1b3849deab03f11560d8564d1f588acc069c1`).
...
The final benchmark pins Overture release `2026-09-23.0` / schema `v2.0.0` and records Foursquare Open Source Places Iceberg snapshot `2325979374271449319` at `2026-09-15 20:07:45.157000` / schema marker `FSQ_OS_CURRENT`. FSQ access used a locally ignored Places Portal token through an in-memory DuckDB secret; no credential value was logged, reported or committed. Both providers completed against the same lock `1.0.3`, six circles, 60-row gold set, mappings, normalization, thresholds and fixed sample seed.
