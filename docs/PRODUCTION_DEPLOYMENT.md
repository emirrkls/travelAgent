...
- outbound DNS, HTTPS/ACME, registry access, and managed-database access as required
- initial single-node estimate: 2 vCPU, 4 GB RAM, and 40 GB SSD for a small workload; measure heap, connection count, query latency, database size, and photo traffic before resizing
- synchronized UTC clock (NTP); containers and JVM are configured for UTC

Photos are currently URL strings in PostgreSQL (`places.photos`, `visits.photos`, and cover-image columns); the audited backend does not upload or store image binaries. Availability, retention, privacy, and deletion of externally hosted photos therefore belong to the external image provider. Do not place photo binaries in the container filesystem.
...
Replace every `CHANGE_ME`. Generate independent random database and JWT secrets. Set `PHOKARTA_CORS_ALLOWED_ORIGINS` to explicit HTTPS origins (no wildcard with credentials). `APP_ENVIRONMENT` is required and `PHOKARTA_RELEASE` should identify the immutable image SHA in structured logs.
...
1. Confirm backup and restore-drill status, free disk, database health, DNS, secrets, image digest, and migration review.
...
6. Verify TLS, expected security headers, login/refresh/logout, an authenticated read, latency, errors, and database connections. Run write-path smoke tests only in staging unless a production mutation is explicitly approved.
...
- [ ] Tested migrations and restore drill on a recent backup
- [ ] Confirmed 7 daily/4 weekly off-host backup policy
